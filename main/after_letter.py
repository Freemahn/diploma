...
m_, v0_ = 2, 1
H = H0
n = 5 # number of args a
k = 2 # number of args a
a = args_a[:n]
d = args_d[:k]
...
def plot_vx():
    ...
    if len(result) == 0:
        ...
        print("vx(0,y) = {}".format(substitute(vx1, [x], [-H0_ ])))
        print("vx(0,y) = {}".format(substitute(vx1, [x], [H0_ ])))
        ...
        p2 = plot(
            vx3,
            xlim=(-H0_ - 0.1, H0_ + 0.1), ylim=(-0 - 0.1, (1.5 + 0.1)), ylabel='vx(0,y)', show=false, line_color='black')
        p21 =  plot(substitute(vx1, [x], [-L_ + 2 ]),show=false, line_color='brown')
        p22 =  plot(substitute(vx1, [x], [-L_/2]), show=false, line_color='blue')
        p23 =  plot(substitute(vx1, [x], [L_ / 2 ]),show=false, line_color='green')
        p24 =  plot(substitute(vx1, [x], [L_-2]), show=false, line_color='yellow')
        p2.extend(p21)
        p2.extend(p22)
        p2.extend(p23)
        p2.extend(p24)
...
def plot_vy():
    ...
    if len(result) == 0:
        ...
        p2 = plot(
            vy3,
            xlim=(-H0_ - 0.1, H0_ + 0.1), ylim=(-0 - 0.1, (1.5 + 0.1)), ylabel='vx(0,y)', show=false,
            line_color='black')
